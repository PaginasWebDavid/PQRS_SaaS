...
import { pqrsResourceScopeForUser } from "@/domains/pqrs/pqrs-security";
...
export function isPqrsTakenByAdministration(pqrs: TakenPqrs) {
  return (
    pqrs.estado !== "EN_ESPERA" ||
    Boolean(pqrs.fechaPrimerContacto) ||
    Boolean(pqrs.gestionadoPorId) ||
    Boolean(pqrs.numeroRadicacion)
  );
}

export function pqrsScopeForUser({
  tenantId,
  userId,
  role,
}: {
...
export function canResidentAccessPqrs({
  ownerId,
  userId,
}: {
  ownerId: string | null;
...
export { pqrsResourceScopeForUser };
