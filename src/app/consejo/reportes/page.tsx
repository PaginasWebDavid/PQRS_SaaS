...
const ESTADO_LABEL: Record<Estado, string> = { EN_ESPERA: 'En espera', EN_PROGRESO: 'En proceso', TERMINADO: 'Terminada' };
...
export default function ConsejoReportesPage() {
  ...
  useEffect(() => {
    fetch('/api/pqrs/categories?includeInactive=true', { cache: 'no-store' })
  ...
  const resumenCards = data ? [
    ...
    { label: 'En espera', metric: data.resumen.abiertas, desc: 'PQRS que aún no han tenido primer contacto del equipo administrativo.' },
    ...
    { label: 'Terminadas', metric: data.resumen.cerradas, desc: 'PQRS que ya fueron cerradas dentro del periodo seleccionado.' },
  ...
  return (
    <AdminShell navItems={CONSEJO_NAV} activeKey="reportes" userName="Consejo" userRole="Consejo de Administración" initials="CM" mobileTitle="Reportes">
              ...
              <tbody>
                {filteredTableRows.slice(0, 200).map((r) => (
                  <tr key={r.id} style={{ borderTop: `1px solid ${COLORS.borderSoft}` }}>
                    ...
                    <td><Link href={`/consejo?id=${r.id}`} style={{ fontSize: 11.5, fontWeight: 700, color: COLORS.navy, whiteSpace: 'nowrap' }}>Ver en PQRS ›</Link></td>
                  </tr>
                ))}
