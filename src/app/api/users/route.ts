...
export async function GET(req: NextRequest) {
  ...
  if (role && !INVITABLE_TENANT_ROLES.includes(role as Role)) {
  ...
  if (paginated && (!Number.isInteger(page) || page < 1 || page > 100000)) {
  ...
  if (
    bloqueRaw &&
    (!Number.isInteger(bloque) || (bloque as number) < 1 || (bloque as number) > 999)
  ) {
  ...
  try {
    ...
    if (!paginated) return NextResponse.json(users);
    return NextResponse.json({
      data: users,
      pagination: {
        page,
