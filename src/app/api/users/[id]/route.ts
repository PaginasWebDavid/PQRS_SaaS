...
export async function GET(
  ...
  try {
    const membership = await prisma.tenantMembership.findUnique({
      where: { userId_tenantId: { userId: params.id, tenantId: access.tenantId } },
      select: {
    ...
    const [pqrsTotal, pqrsTerminadas] = await Promise.all([
      prisma.pqrs.count({ where: { tenantId: access.tenantId, creadoPorId: params.id } }),
      prisma.pqrs.count({ where: { tenantId: access.tenantId, creadoPorId: params.id, estado: "TERMINADO" } }),
    ]);
