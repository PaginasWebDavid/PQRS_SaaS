...
import { SuperAdminShell, NavGroup } from '@/components/SuperAdminShell';
import { Sheet, CloseButton } from '@/components/Sheet';
import { Toast, useToast } from '@/components/Toast';
import { COLORS, RADIUS, badgeStyle, tabStyle, toggleTrackStyle, toggleDotStyle } from '@/lib/tokens';
...
type Tenant = {
  id: number; name: string; city: string; units: number; plan: string; group: TenantGroup;
...
const INITIAL_TENANTS: Tenant[] = [
  { id: 1, name: 'Parque Residencial Calle 100', city: 'Bogotá', units: 312, plan: '301–500', group: 'active', adminName: 'Ana Ruiz', adminEmail: 'ana.ruiz@parque100.com', adminPhone: '+57 300 123 4567', startDate: '12 ene 2025', pqrsTotal: 412, pqrsOpen: 24, licenseEnd: '15 ago 2026', paymentStatus: 'al_dia', moraDays: 0 },
  { id: 2, name: 'Reserva del Bosque', city: 'Medellín', units: 96, plan: '50–100', group: 'active', adminName: 'Felipe Rojas', adminEmail: 'f.rojas@reservabosque.com', adminPhone: '+57 301 555 2211', startDate: '03 mar 2025', pqrsTotal: 88, pqrsOpen: 6, licenseEnd: '02 sep 2026', paymentStatus: 'al_dia', moraDays: 0 },
  { id: 3, name: 'Torres del Prado', city: 'Cali', units: 540, plan: '501+', group: 'active', adminName: 'Marta Uribe', adminEmail: 'm.uribe@torresdelprado.com', adminPhone: '+57 302 888 9090', startDate: '20 jun 2024', pqrsTotal: 705, pqrsOpen: 31, licenseEnd: '28 jul 2026', paymentStatus: 'al_dia', moraDays: 0 },
  { id: 4, name: 'Mirador de la Sabana', city: 'Bogotá', units: 210, plan: '101–300', group: 'active', adminName: 'Hernán Cifuentes', adminEmail: 'h.cifuentes@miradorsabana.com', adminPhone: '+57 310 222 3344', startDate: '14 sep 2024', pqrsTotal: 250, pqrsOpen: 40, licenseEnd: '01 jul 2026', paymentStatus: 'mora', moraDays: 14 },
  { id: 5, name: 'Alameda Real', city: 'Bogotá', units: 78, plan: '50–100', group: 'suspended', adminName: 'Patricia Gómez', adminEmail: 'p.gomez@alamedareal.com', adminPhone: '+57 315 444 5566', startDate: '02 nov 2024', pqrsTotal: 60, pqrsOpen: 0, licenseEnd: '—', paymentStatus: 'mora', moraDays: 45 },
  { id: 6, name: 'Portal del Country', city: 'Bogotá', units: 130, plan: '101–300', group: 'trial', adminName: 'Diego Salazar', adminEmail: 'd.salazar@portalcountry.com', adminPhone: '+57 300 999 1122', startDate: '26 jun 2026', pqrsTotal: 8, pqrsOpen: 3, licenseEnd: 'Prueba vence 10 jul 2026', paymentStatus: 'al_dia', moraDays: 0 },
];

const INITIAL_TIERS = [
  { id: 1, from: '50', to: '100', price: '$650.000/mes' },
  { id: 2, from: '101', to: '300', price: '$1.180.000/mes' },
  { id: 3, from: '301', to: '500', price: '$1.850.000/mes' },
  { id: 4, from: '501', to: '+', price: 'Cotización personalizada' },
];

const KPIS = [
  { label: 'Total conjuntos', value: '184', color: '#1D1D1F' }, { label: 'Activos', value: '178', color: COLORS.success },
  { label: 'Suspendidos', value: '6', color: COLORS.textSecondaryAlt }, { label: 'Trial', value: '9', color: COLORS.navy },
  { label: 'Usuarios totales', value: '12.4k', color: COLORS.navy }, { label: 'PQRS abiertas', value: '340', color: COLORS.warning },
];
...
export default function DashboardSuperAdminPage() {
  ...
  const suspend = (id: number) => { setTenants((ts) => ts.map((t) => t.id === id ? { ...t, group: 'suspended' } : t)); showToast('Conjunto suspendido ✓'); addAudit('⏸', `Suspendió el conjunto`); };
  const reactivate = (id: number) => { setTenants((ts) => ts.map((t) => t.id === id ? { ...t, group: 'active', paymentStatus: 'al_dia', moraDays: 0 } : t)); showToast('Conjunto reactivado ✓'); addAudit('▶', `Reactivó el conjunto`); };
  const cancelTenant = (id: number) => { setTenants((ts) => ts.map((t) => t.id === id ? { ...t, group: 'cancelled' } : t)); setSelectedId(null); setConfirmingCancel(false); showToast('Conjunto cancelado'); addAudit('✕', 'Canceló un conjunto'); };

  const submitCreate = () => {
    if (!newName.trim() || !newUnits) return;
    setCreatePhase('progress'); setCreateStep(0);
    const steps = 5;
    const tick = (i: number) => setTimeout(() => {
      if (i < steps) { setCreateStep(i); tick(i + 1); }
      else {
        const units = parseInt(newUnits, 10) || 0;
        const plan = units <= 100 ? '50–100' : units <= 300 ? '101–300' : units <= 500 ? '301–500' : '501+';
        setTenants((ts) => [{ id: Math.max(0, ...ts.map((t) => t.id)) + 1, name: newName, city: newCity || '—', units, plan, group: 'trial', adminName: newAdminName || '—', adminEmail: newAdminEmail, adminPhone: '—', startDate: 'jul 2026', pqrsTotal: 0, pqrsOpen: 0, licenseEnd: 'Prueba de 15 días', paymentStatus: 'al_dia', moraDays: 0 }, ...ts]);
        setCreatePhase('done'); addAudit('+', `Creó el conjunto "${newName}"`);
      }
    }, 450);
    tick(1);
  };
    ...
    <SuperAdminShell navGroups={navGroups} activeKey={nav} mobileTitle="Plataforma">
      ...
      {nav === 'resumen' && (
        <div className="apl-up">
          ...
          <p style={{ fontSize: 13.5, color: COLORS.textSecondary, margin: '0 0 22px' }}>184 conjuntos administrados</p>
          ...
          <div style={{ background: '#FFFFFF', border: `1px solid ${COLORS.border}`, borderRadius: 18, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: `1px solid ${COLORS.borderSoft}` }}>
              ...
              <span onClick={() => setNav('conjuntos')} style={{ fontSize: 12.5, fontWeight: 700, color: COLORS.navy, cursor: 'pointer' }}>Ver todos ›</span>
            </div>
      ...
      {nav === 'config' && (
        <div className="apl-up" style={{ maxWidth: 640, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: COLORS.bgCard, borderRadius: 18, padding: 22 }}>
            ...
            {['Adjuntar evidencias en PQRS', 'Auto-registro de residentes'].map((label, i) => {
              const [on, setOn] = [true, () => {}]; // conectar a estado real
              return (
            ...
            {confirmingCancel && (
              <div style={{ border: `1.5px solid #F3D9B1`, background: COLORS.warningSoft, borderRadius: 14, padding: 18, marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <div onClick={() => cancelTenant(selected.id)} style={{ background: COLORS.warning, color: '#FFFFFF', fontSize: 13, fontWeight: 700, padding: '10px 16px', borderRadius: RADIUS.pill, cursor: 'pointer' }}>Sí, cancelar</div>
